# tests/test_compared_twist_operation.py
...
def test_compared_twist_operation_reports_volume_change(monkeypatch, tmp_path) -> None:
    ...
    result = compared_twist_operation.apply_compared_twist_operation(
    ...
    assert result.comparison.volume_removed_mm3 > 0
    assert result.comparison.volume_removed_percent > 0
    assert result.comparison.output_report.watertight is True
